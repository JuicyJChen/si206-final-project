...
def get_lat_lon(city_name):
    response = requests.get(f"https://geocoding-api.open-meteo.com/v1/search?name={city_name}&count=1&format=json")
...
def insert_city_data(city_name):
    latitude, longitude = get_lat_lon(city_name)
    if latitude is not None and longitude is not None:
        cursor.execute("INSERT OR IGNORE INTO locations (city_name, latitude, longitude) VALUES (?, ?, ?)",
...
cities = [
    "Ann Arbor", "New York City", "Tokyo", "Delhi", "Shanghai", "São Paulo", "Mexico City", "Cairo", "Mumbai", "Beijing", "Dhaka", "Osaka",
    ...
    "Hangzhou", "Foshan", "Shenyang", "Riyadh", "Baghdad", "Santiago", "Surat", "Madrid", "Suzhou", "Pune",
    "Harbin", "Houston", "Dallas", "Toronto", "Dar es Salaam", "Miami", "Belo Horizonte", "Singapore", "Philadelphia",
    "Atlanta", "Fukuoka", "Khartoum", "Barcelona", "Johannesburg", "Saint Petersburg", "Qingdao", "Dalian", "Washington, D.C.",
    "Yangon", "Alexandria", "Jinan", "Guadalajara", "Boston", "Zhengzhou", "Hanoi", "Ankara", "Monterrey", "Yokohama",
    "Nairobi", "Zhoukou", "Cape Town", "Shijiazhuang", "Changsha", "Kunming", "Taiyuan", "Xiamen", "Hefei", "Changchun", "Sydney", "Nanchang", "Zhumadian", "Abington", "Ypsilanti", "Chicago"
...
for city_name in cities:
    if city_name not in existing_cities:
        success = insert_city_data(city_name)
        if success:
            cities_processed += 1
