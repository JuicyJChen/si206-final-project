...
def get_existing_dates(db_connection):
    ...
    return {
        datetime.strptime(row[0].split()[0], "%Y-%m-%d").date()
    ...
    for sample_date in sample_dates:
        ...
        if "hourly" in data:
            ...
            try:
                ...
                cursor.execute(
                    "INSERT INTO temperature (date, temperature, location_id, latitude, longitude) VALUES (?, ?, ?, ?, ?)",
                    (date, temperature, location_id, latitude, longitude),
                )
                temp_id = cursor.lastrowid

                # Insert precipitation data
                cursor.execute(
                    "INSERT INTO precipitation (date, rain, snowfall, temp_id, location_id, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?)",
        ...
        if stock_data:
            ...
            try:
                cursor.execute(
                    "INSERT INTO stock_data (date, open_price, close_price, high_price, low_price, volume) VALUES (?, ?, ?, ?, ?, ?)",
                    (
        ...
        if air_quality_data and "hourly" in air_quality_data:
            ...
            try:
                ...
                longitude = cursor.fetchone()[0]
                cursor.execute(
                    "INSERT INTO air_quality (temp_id, date, pm10, pm2_5, carbon_monoxide, nitrogen_dioxide, sulphur_dioxide, ozone, location_id, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (
                        temp_id,
                        sample_date.strftime("%Y-%m-%d"),
                        pm10,
